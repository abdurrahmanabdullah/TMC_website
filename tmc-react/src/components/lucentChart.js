...
const LucentChartComponent = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          "https://api.strapi.transworldbd.com/api/lucent-chart"
        );
  ...
  const { image, video, element1, element2, logo, footer } =
    lucentChartData.attributes;

  return (
    <div style={{ padding: "40px" }}>
      <h1 style={{ fontWeight: "normal", textAlign: "center" }}>
        {lucentChartData.attributes.introductionpart}
      </h1>
      {/* Display the image name */}

      <img
        src={`https://api.strapi.transworldbd.com${lucentChartData.attributes.image.data.attributes.url}`}
      ...
      {video && (
        <div style={{ marginTop: "90px", textAlign: "center" }}>
          <ReactPlayer
            url={`https://api.strapi.transworldbd.com${video.data.attributes.url}`}
            width="560px"
            height="315px"
            controls
            style={{ margin: "0 auto" }}
      ...
      <div className="additional-images">
        {logo.data.map((logoItem, index) => (
          <img
            key={logoItem.id}
            src={`
https://api.strapi.transworldbd.com${logoItem.attributes.url}`}
